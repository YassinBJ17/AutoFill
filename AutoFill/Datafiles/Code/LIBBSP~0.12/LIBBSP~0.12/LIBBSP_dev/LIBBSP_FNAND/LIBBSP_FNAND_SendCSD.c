...
/* Private function
 * Get the CSD register from the memory. Memory should be in STBY state
 * Parameters : (uint32_t*) p_CSD -> Pointer on uint32_t array of 4 elements
 * Return : TE_LIBBSP_FNAND_ERROR_OP
 */
TE_LIBBSP_FNAND_ERROR_OP SendCSD(uint32_t p_CSD[C_FNAND_CSD_BUFFER_SIZE]) /* %RELAX<SYNCHRONe_C_Code_8.18> p_CSD is output */
{
   ...
   P_LIBBSP_FNAND_Op->CMD = C_FNAND_SEND_CSD;
   ...
   if(P_LIBBSP_FNAND_StatusCard->B.CURRENT_STATE == (uint32_t)E_LIBBSP_FNAND_CARD_STATE_STBY) /* %RELAX<SYNCHRONe_C_Code_10.7> enum convertion to uint32_t do not imply loss of precision */
   {
      ...
      v_Cmd.CMD_PARAM.B.CMDINX = C_FNAND_SEND_CSD;       /* CMD9 - SEND_CSD */
      ...
      {
           for(v_i = 0ul; v_i < C_FNAND_CSD_BUFFER_SIZE; v_i++)
           {
              p_CSD[v_i] = P_LIBBSP_FNAND_Op->CMD_RSP[v_i];
           }
      }
      else
      {
