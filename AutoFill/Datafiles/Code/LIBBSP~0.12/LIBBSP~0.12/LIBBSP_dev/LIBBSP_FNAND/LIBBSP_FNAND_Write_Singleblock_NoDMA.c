...
/****************************************************************************************/
#include "LIBBSP_FNAND_Write_Singleblock_NoDMA.h"
...
/* Private function
 * Write one block (512 bytes) in memory without DMA at v_SectorAddr.
...
TE_LIBBSP_FNAND_ERROR_OP Write_Singleblock_NoDMA(const uint32_t v_SectorAddr, const uint32_t* const p_Data)
{
   ...
   P_LIBBSP_FNAND_Op->CMD = C_FNAND_WRITE_SINGLE_BLOCK;
   ...
   if((P_LIBBSP_FNAND_StatusCard->B.READY_FOR_DATA == 0x00ul) ||
              (P_LIBBSP_FNAND_Host->PRSSTAT.B.DLA == 0x01ul))
   {
   ...
   if(v_Error == E_LIBBSP_FNAND_NO_ERROR)
   {
      ...
      v_Cmd.CMD_PARAM.B.CMDINX = C_FNAND_WRITE_SINGLE_BLOCK; /* CMD24 - WRITE_BLOCK  */
