...
/****************************************************************************************/
#include "LIBBSP_SPI_Write6Bytes.h"
...
/*==== Services ========================================================================*/
void LIBBSP_SPI_Write6Bytes(const TE_LIBBSP_SPI_Device      SpiDevice,
                            ...
                            TE_LIBBSP_SPI_ErrorCode * const v_SpiErr)
{
   ...
   *v_SpiErr = E_LIBBSP_SPI_OK;
   ...
   V_SPI_Regs->spitf.u8 = Buffer[4];
   V_SPI_Regs->spitf.u8 = Buffer[5];
   /*}}RELAX<SYNCHRONe_C_Code_17.4> */
   /* Configure the SPI bus to send 6 chars */
   switch (SpiDevice)
   {
      case E_LIBBSP_SPI_FLASH_PROGRAM:
         V_SPI_Regs->spcom.R = (uint32_t)C_SPI_BUS_FLASH_PROGRAM_6CHAR;
         break;
      case E_LIBBSP_SPI_MRAM:
         V_SPI_Regs->spcom.R = (uint32_t)C_SPI_BUS_MRAM_6CHAR;
         break;
      case E_LIBBSP_SPI_FLASH_BITE:
         V_SPI_Regs->spcom.R = (uint32_t)C_SPI_BUS_FLASH_BITE_6CHAR;
         break;
      default:
         V_SPI_Regs->spcom.R = (uint32_t)C_SPI_BUS_DEFAULT_6CHAR;
         break;
   }
