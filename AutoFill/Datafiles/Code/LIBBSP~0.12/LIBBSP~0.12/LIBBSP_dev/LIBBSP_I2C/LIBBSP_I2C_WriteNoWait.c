...
/****************************************************************************************/
#include "LIBBSP_I2C_WriteNoWait.h"
...
TE_LIBBSP_I2C_ErrorCode LIBBSP_I2C_WriteNoWait(	const TE_LIBBSP_I2C_Bus v_devID,
					...
					TE_LIBBSP_I2C_CurrentStep* const v_step)
{
	...
	{
		v_errorCode = E_LIBBSP_I2C_WRITE_OVERSIZED;
	}
	else
	{
		...
		if (*v_step == E_LIBBSP_I2C_IDLE)
		{
			...
			{
				V_I2C_BuffWrite[v_devID][v_i] = p_buffer[v_i];
			}

			V_I2C_WriteSize[v_devID] = v_size;
			...
			if (P_I2C_RegAddress[v_devID].i2csr.I2CSR.MBB == (uint8_t)0)
			{
				...
				P_I2C_RegAddress[v_devID].i2cdr.I2CDR.DATA = (uint8_t)((uint8_t)(v_slave_adress << 1) | (uint8_t)0);   /* Device address + write */

				*v_step = E_LIBBSP_I2C_WAIT_FOR_SLV_CMD_W_ACK;
			}
			else
			{
