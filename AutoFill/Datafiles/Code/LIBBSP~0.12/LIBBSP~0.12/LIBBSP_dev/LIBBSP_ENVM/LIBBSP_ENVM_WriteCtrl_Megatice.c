...
void LIBBSP_ENVM_WriteCtrl_Megatice(const uint32_t address, const uint32_t size, const uint8_t * const buffer, TE_LIBBSP_ENVM_ErrorCode * const v_ret)
{
	...
	while (v_address < (address + size))
	{
		if (v_address % C_LIBBSP_ENVM_PAGESIZE != 0ul)
		{
			...
			LIBBSP_ENVM_Read_Megatice(v_address, C_LIBBSP_ENVM_PAGESIZE, v_buff, v_ret);
		}
