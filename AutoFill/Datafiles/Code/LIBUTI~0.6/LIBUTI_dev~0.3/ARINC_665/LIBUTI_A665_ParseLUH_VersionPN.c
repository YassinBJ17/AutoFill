...
/*Version check*/
    ...
    {
#ifdef A665_PART2
    case C_LOAD_FILE_FORMAT_VERSION_2:
#endif
#ifdef A665_PART3
    case C_LOAD_FILE_FORMAT_VERSION_3:
#endif
        break;
...
/*Load PN*/
    v_LoadPN_Offset = p_LUH_ConstantSizeFirstFields->s_PointerToLoadPN * 2ul;
    ...
    *p_LoadPN_Length = *((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_LoadPN_Offset)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
    if ( *p_LoadPN_Length > (uint16_t)C_PN_MAX_LENGTH )
    {
    ...
    LibC_Memcpy( 
        p_LoadPN,
        (uint8_t*)((uint32_t)p_LUH_RAM_Address + v_LoadPN_Offset + 2ul), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
