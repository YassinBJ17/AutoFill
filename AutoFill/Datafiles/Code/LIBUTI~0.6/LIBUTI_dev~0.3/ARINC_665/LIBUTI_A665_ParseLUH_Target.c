...
)
{
    ...
    /*Target HW IDs*/
    v_TargetHW_ID_ListOffset = p_LUH_ConstantSizeFirstFields->s_PointerToTargetHW_ID_List * 2ul;
    ...
    *p_NumberOfTargetHW_IDs = (uint32_t)*((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_TargetHW_ID_ListOffset)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
    ...
    p_TargetHW_ID_Iterator = (uint8_t*)((uint32_t)p_LUH_RAM_Address + v_TargetHW_ID_ListOffset + 2ul); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
/* {{RELAX<SYNCHRONe_C_Code_17.4> Need to access the table dynamically as size is not known at compile time */
    for ( v_index = 0ul; v_index < *p_NumberOfTargetHW_IDs; v_index++ )
    {
        ...
        if ( p_TargetHW_IDs_WithLengths[v_index].Length > (uint16_t)C_TARGET_HW_ID_MAX_LENGTH )
        {
        ...
        LibC_Memcpy(
            ...
            (uint8_t*)((uint32_t)p_TargetHW_ID_Iterator + 2ul), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
            (uint32_t)p_TargetHW_IDs_WithLengths[v_index].Length );

        p_TargetHW_ID_Iterator = (uint8_t*)((uint32_t)p_TargetHW_ID_Iterator + (uint32_t)p_TargetHW_IDs_WithLengths[v_index].Length + 2ul); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
    }
