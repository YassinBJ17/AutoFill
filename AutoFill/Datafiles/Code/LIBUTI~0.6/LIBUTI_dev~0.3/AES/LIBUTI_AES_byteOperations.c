/*----------------------------------------------------------------------------*/
/* File name  : LIBUTI_AES_byteOperations.c
 * CSC name   :
 *
 * Description:
 *
 * Configuration management identification:
 *    %full_name:  sknNEXT#1/csrc/LIBUTI_AES_byteOperations.c/2 %
 *    Creation: FT131472            on Thu Mar 25 17:52:30 CET 2021
 *
 * Copyright (c) 2019,
...
void xor_buf(const uint8_t in[], uint8_t out[], uint32_t const len)  /* %RELAX<SYNCHRONe_C_Code_8.12> The size of the arrays are not known at this point */
{
   uint32_t idx;

   for (idx = (uint32_t) 0; idx < len; idx++)
   {
      out[idx] ^= in[idx];
   }
}
...
uint32_t SubWord(uint32_t const word)
{
   ...
   result = (uint32_t)aes_sbox[(word >> 4) & ((uint32_t) 0x0000000F)][word & ((uint32_t) 0x0000000F)];
   result += (uint32_t)aes_sbox[(word >> 12) & ((uint32_t) 0x0000000F)][(word >> 8) & ((uint32_t) 0x0000000F)] << 8;
   result += (uint32_t)aes_sbox[(word >> 20) & ((uint32_t) 0x0000000F)][(word >> 16) & ((uint32_t) 0x0000000F)] << 16;
   result += (uint32_t)aes_sbox[(word >> 28) & ((uint32_t) 0x0000000F)][(word >> 24) & ((uint32_t) 0x0000000F)] << 24;
   return(result);
}
...
void AddRoundKey(uint8_t state[4][4], const uint32_t w[4])
{
   ...
   /* So >> and cast to uint8_t are used to split the uint32_t variable in 4 uint8_t*/
   /* Here the loss of precision is wanted*/
   /* Subkey 1 */
   subkey[0] = (uint8_t) (w[0] >> 24);
   subkey[1] = (uint8_t) (w[0] >> 16);
   subkey[2] = (uint8_t) (w[0] >> 8);
   subkey[3] = (uint8_t) (w[0]);
   state[0][0] ^= subkey[0];
   state[1][0] ^= subkey[1];
   state[2][0] ^= subkey[2];
   state[3][0] ^= subkey[3];
   /* Subkey 2 */
   subkey[0] = (uint8_t) (w[1] >> 24);
   subkey[1] = (uint8_t) (w[1] >> 16);
   subkey[2] = (uint8_t) (w[1] >> 8);
   subkey[3] = (uint8_t) (w[1]);
   state[0][1] ^= subkey[0];
   state[1][1] ^= subkey[1];
   state[2][1] ^= subkey[2];
   state[3][1] ^= subkey[3];
   /* Subkey 3 */
   subkey[0] = (uint8_t) (w[2] >> 24);
   subkey[1] = (uint8_t) (w[2] >> 16);
   subkey[2] = (uint8_t) (w[2] >> 8);
   subkey[3] = (uint8_t) (w[2]);
   state[0][2] ^= subkey[0];
   state[1][2] ^= subkey[1];
   state[2][2] ^= subkey[2];
   state[3][2] ^= subkey[3];
   /* Subkey 4 */
   subkey[0] = (uint8_t) (w[3] >> 24);
   subkey[1] = (uint8_t) (w[3] >> 16);
   subkey[2] = (uint8_t) (w[3] >> 8);
   subkey[3] = (uint8_t) (w[3]);
   state[0][3] ^= subkey[0];
   state[1][3] ^= subkey[1];
   state[2][3] ^= subkey[2];
   state[3][3] ^= subkey[3];
...
{
	state[0][0] = aes_sbox[state[0][0] >> 4][state[0][0] & ((uint8_t) 0x0F)];
	state[0][1] = aes_sbox[state[0][1] >> 4][state[0][1] & ((uint8_t) 0x0F)];
	state[0][2] = aes_sbox[state[0][2] >> 4][state[0][2] & ((uint8_t) 0x0F)];
	state[0][3] = aes_sbox[state[0][3] >> 4][state[0][3] & ((uint8_t) 0x0F)];
	state[1][0] = aes_sbox[state[1][0] >> 4][state[1][0] & ((uint8_t) 0x0F)];
	state[1][1] = aes_sbox[state[1][1] >> 4][state[1][1] & ((uint8_t) 0x0F)];
	state[1][2] = aes_sbox[state[1][2] >> 4][state[1][2] & ((uint8_t) 0x0F)];
	state[1][3] = aes_sbox[state[1][3] >> 4][state[1][3] & ((uint8_t) 0x0F)];
	state[2][0] = aes_sbox[state[2][0] >> 4][state[2][0] & ((uint8_t) 0x0F)];
	state[2][1] = aes_sbox[state[2][1] >> 4][state[2][1] & ((uint8_t) 0x0F)];
	state[2][2] = aes_sbox[state[2][2] >> 4][state[2][2] & ((uint8_t) 0x0F)];
	state[2][3] = aes_sbox[state[2][3] >> 4][state[2][3] & ((uint8_t) 0x0F)];
	state[3][0] = aes_sbox[state[3][0] >> 4][state[3][0] & ((uint8_t) 0x0F)];
	state[3][1] = aes_sbox[state[3][1] >> 4][state[3][1] & ((uint8_t) 0x0F)];
	state[3][2] = aes_sbox[state[3][2] >> 4][state[3][2] & ((uint8_t) 0x0F)];
	state[3][3] = aes_sbox[state[3][3] >> 4][state[3][3] & ((uint8_t) 0x0F)];
}

void InvSubBytes(uint8_t state[4][4])
{
	state[0][0] = aes_invsbox[state[0][0] >> 4][state[0][0] & ((uint8_t) 0x0F)];
	state[0][1] = aes_invsbox[state[0][1] >> 4][state[0][1] & ((uint8_t) 0x0F)];
	state[0][2] = aes_invsbox[state[0][2] >> 4][state[0][2] & ((uint8_t) 0x0F)];
	state[0][3] = aes_invsbox[state[0][3] >> 4][state[0][3] & ((uint8_t) 0x0F)];
	state[1][0] = aes_invsbox[state[1][0] >> 4][state[1][0] & ((uint8_t) 0x0F)];
	state[1][1] = aes_invsbox[state[1][1] >> 4][state[1][1] & ((uint8_t) 0x0F)];
	state[1][2] = aes_invsbox[state[1][2] >> 4][state[1][2] & ((uint8_t) 0x0F)];
	state[1][3] = aes_invsbox[state[1][3] >> 4][state[1][3] & ((uint8_t) 0x0F)];
	state[2][0] = aes_invsbox[state[2][0] >> 4][state[2][0] & ((uint8_t) 0x0F)];
	state[2][1] = aes_invsbox[state[2][1] >> 4][state[2][1] & ((uint8_t) 0x0F)];
	state[2][2] = aes_invsbox[state[2][2] >> 4][state[2][2] & ((uint8_t) 0x0F)];
	state[2][3] = aes_invsbox[state[2][3] >> 4][state[2][3] & ((uint8_t) 0x0F)];
	state[3][0] = aes_invsbox[state[3][0] >> 4][state[3][0] & ((uint8_t) 0x0F)];
	state[3][1] = aes_invsbox[state[3][1] >> 4][state[3][1] & ((uint8_t) 0x0F)];
	state[3][2] = aes_invsbox[state[3][2] >> 4][state[3][2] & ((uint8_t) 0x0F)];
	state[3][3] = aes_invsbox[state[3][3] >> 4][state[3][3] & ((uint8_t) 0x0F)];
}
