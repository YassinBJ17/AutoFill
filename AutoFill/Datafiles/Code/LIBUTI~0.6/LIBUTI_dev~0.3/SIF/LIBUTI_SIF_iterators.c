/*----------------------------------------------------------------------------*/
/* File name  : LIBUTI_SIF_iterators.c
 * CSC name   : LIBUTI_SIF
 *
 * Description:
 *
 * Configuration management identification:
 *    %full_name:  sknNEXT#1/csrc/LIBUTI_SIF_iterators.c/3 %
...
TE_LIBUTI_SIF_status LIBUTI_SIF_get_next_target(ts_LIBUTI_SIF_target * const target, ts_LIBUTI_SIF_iterator * const iter)
{
    ...
    {
        LIBUTI_MEM_MemSet( (uint8_t *) target, (uint8_t)0, sizeof(ts_LIBUTI_SIF_target)); /*%RELAX<SYNCHRONe_C_Code_10.7, SYNCHRONe_C_Code_11.1> Pointer conversion : Use generic pointer for LIBUTI_MEM_MemSet */
...
TE_LIBUTI_SIF_status LIBUTI_SIF_get_next_file(ts_LIBUTI_SIF_file_info * const file, ts_LIBUTI_SIF_iterator * const iter)
{
    ...
    if (iter->data.position < iter->data.header_length)
    {
        LIBUTI_MEM_MemSet((uint8_t *)file, (uint8_t)0, sizeof(ts_LIBUTI_SIF_file_info)); /*%RELAX<SYNCHRONe_C_Code_10.7, SYNCHRONe_C_Code_11.1> Pointer conversion : Use generic pointer for LIBUTI_MEM_MemSet */
        next_ptr = LIBUTI_SIF_get_long(&(iter->data));
        iter->item_count--;
        if (next_ptr == (uint32_t)0 || iter->item_count == (uint16_t)0)
        {
