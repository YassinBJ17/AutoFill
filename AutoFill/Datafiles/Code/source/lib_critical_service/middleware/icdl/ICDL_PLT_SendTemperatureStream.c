...
#define K_TEMP_CMD_UDP_PORTS_SRC  49700
#define K_TEMP_CMD_UDP_PORTS_DST  49700


/* ---------- provided operation bodies: ------------------------------------ */
void ICDL_PLT_SendTemperatureStream ( CMN_SYSTEM_ADDR_t const p_buff_addr,
...

{
    ...
    */
   t_command   *eventBlock_cmd = ICDL_LG_TriggerCommand( ID_CMD_TEMP_STREAM, (uint16_t)p_buff_size );

   /*
    * Set channel transmission
    */
   eventBlock_cmd->st_descriptor->ethUdpInfo.s_SrcPort = K_TEMP_CMD_UDP_PORTS_SRC;
   eventBlock_cmd->st_descriptor->ethUdpInfo.s_DstPort = K_TEMP_CMD_UDP_PORTS_DST;

   {
      boolean_t bool_channel_prepared = ICDL_LG_PrepareTxChannel( eventBlock_cmd );
      ...
      {
         ICDL_LG_ComputeMsgArgSet( (int8_t *)&p_nb_sensors, sizeof (uint32_t), eventBlock_cmd );
         ICDL_LG_ComputeMsgDataSet( (int8_t *)p_buff_addr, (uint32_t)p_buff_size, eventBlock_cmd );
