...
/* */
void MAC_ETH_SET_MAC_SRC_MASK  ( CMN_IOSP_ID_t const p_conf_ip_id,
                                 const MAC_ETH_MACAddr_t * const p_mac_mask_addr )
{
   ...
   pt_mac_eth_rx_filtre_reg->s_ETH_RX_FILTRE_SRC_ADR_MASK_H_REG = ( ((reg32_t)(*p_mac_mask_addr)[ 0 ]) << 8u ) |
                                                                  ( ((reg32_t)(*p_mac_mask_addr)[ 1 ]) << 0u );

   pt_mac_eth_rx_filtre_reg->s_ETH_RX_FILTRE_SRC_ADR_MASK_L_REG = ( ((reg32_t)(*p_mac_mask_addr)[ 2 ]) << 24u ) |
