 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Tx cmd write data function
 * -------------------------------------------------------------------------- */
...
/* */
void MBX_FIFO_TX_CMD_WRITE_DATA ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id,
                                  tu_TX_CMD_DATA_REG * const p_tx_fifo_data )
{
   /* Local Variable Declaration */
   ts_MBX_TX_CMD_REG volatile * pt_fifo_tx_cmd_write_data;
#ifdef CBIT_MBX
   ...
   pt_fifo_tx_cmd_write_data = (ts_MBX_TX_CMD_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_cmd_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_fifo_tx_cmd_write_data->s_TX_CMD_DATA_REG = (reg32_t)p_tx_fifo_data->u_R;
   ...
   if( v_tx_cmd_fifo_status_value.u_B.s_tx_cmd_fifo_overflow == 1 )
   {
      ...
      MBX_FIFO_TX_CMD_WRITE_STATUS_ACK ( p_MBX_FIFO_conf_id, &v_tx_cmd_ack_value );
   ...
   if( v_tx_cmd_fifo_status_value.u_B.s_tx_cmd_fifo_single_biterr == 1 )
   {
      /* 1: Remonter le flag de panne */
      cbit_mbx_status.TX_status |= v_tx_cmd_fifo_status_value.u_B.s_tx_cmd_fifo_single_biterr << as_return_mem_corr;

      /* 2: Acknowledge the status */
      v_tx_cmd_ack_value.u_R = 0;
      v_tx_cmd_ack_value.u_B.s_tx_cmd_fifo_biterr_ack = 1;
      MBX_FIFO_TX_CMD_WRITE_STATUS_ACK ( p_MBX_FIFO_conf_id, &v_tx_cmd_ack_value );
   }

   /* 3: In case of double bitter by FPGA */
   if( v_tx_cmd_fifo_status_value.u_B.s_tx_cmd_fifo_double_biterr == 1 )
   {
      /* 1: Remonter le flag de panne */
      cbit_mbx_status.TX_status |= v_tx_cmd_fifo_status_value.u_B.s_tx_cmd_fifo_double_biterr << as_return_mem_corr;
