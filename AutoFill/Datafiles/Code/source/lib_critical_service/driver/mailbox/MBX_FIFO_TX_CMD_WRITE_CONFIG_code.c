 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Tx cmd write config function
 * -------------------------------------------------------------------------- */
...
/* */
void MBX_FIFO_TX_CMD_WRITE_CONFIG ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id,
                                    tu_TX_CMD_FIFO_CONFIG_REG * const p_status_value )
{
   /* Local Variable Declaration */
   ts_MBX_TX_CMD_REG volatile * pt_fifo_tx_cmd_write_config;
   ...
   pt_fifo_tx_cmd_write_config = (ts_MBX_TX_CMD_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_cmd_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_fifo_tx_cmd_write_config->s_TX_CMD_FIFO_CONFIG_REG = (reg32_t)p_status_value->u_R;
}
