 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : MAC ETH Read Tx Status function
 * -------------------------------------------------------------------------- */
...
/* */
void MAC_ETH_READ_TX_STATUS  ( CMN_IOSP_ID_t const p_conf_ip_id,
                             tu_MAC_ETH_TX_STATUS_REG * const  p_status_value )
{
   /* Local Variable Declaration */
   ts_MAC_ETH_TX_STATUS_REG volatile * pt_mac_eth_tx_status_reg;
   ...
   pt_mac_eth_tx_status_reg = (ts_MAC_ETH_TX_STATUS_REG volatile *)c_MAC_ETH_CFEX_CONFIGURATION_tab[p_conf_ip_id].s_mac_eth_rx_status_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   p_status_value->u_R = (uint32_t)(pt_mac_eth_tx_status_reg->s_ETH_TX_STATUS_REG);
}
