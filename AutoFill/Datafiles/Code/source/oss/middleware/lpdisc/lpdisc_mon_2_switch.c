 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Manage FaultWord for LP with two switches (LP 5, 6, 7, 8, 10)
 * -------------------------------------------------------------------------- */
...
#define C_MASK_LP_2_SWITCH_INVALD    ((uint32_t)0xDE000020)
...
/* Manage FaultWord for LP with two switches (LP 5, 6, 7, 8, 10) */
void lpdisc_mon_2_switch(ts_lpdisk_device * p_device,
                         tu_lp_fault_word * p_fault_word)
{
   ...
   else
   {
      ...
      v_switch_2 = (boolean_t)lpdisc_process_raw(C_RAW_READ, &p_device->s_raw_swi_2);
   ...
   v_flag = ((p_fault_word->u_word & C_MASK_LP_2_SWITCH_INVALD) != (uint32_t)0);
