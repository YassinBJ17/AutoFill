...
{
   ts_ovsp_triggered_test_cmd_frame triggered_test_frame;
   ...
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x10);
   ...
   triggered_test_frame.s_identifier = (uint32_t)OVSP_TRIGGERED_TEST_ID;

   if(triggered_test_on == TRUE)
   {
      triggered_test_frame.s_data_trig_test_on_off = (uint32_t)0x12345678;
   }
   else if(triggered_test_on == FALSE)
   {
      triggered_test_frame.s_data_trig_test_on_off = (uint32_t)0x87654321;
   }

   triggered_test_frame.s_data_test_speed = test_speed;

   triggered_test_frame.s_length = OVSP_FRAME_TOT_LENGTH;

   tmp_can_frame = (ts_can_frame*)(&triggered_test_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = triggered_test_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = triggered_test_frame.s_length;
   ...
   swap_data_cmd(&(triggered_test_frame.s_data_trig_test_on_off), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 1);
}
