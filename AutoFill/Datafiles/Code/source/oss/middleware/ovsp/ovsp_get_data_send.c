...
/* ---------- provided data: ------------------------------------------------ */
ts_ovsp_get_data_frame get_data_frame;
...
void ovsp_get_data_send(uint32_t const can_x_reg_addr, uint32_t const dest_can_ram_addr)
{
   ...
   uint32_t       send_data_id_n;
   uint32_t       i;

   send_data_id_n = 0x0;

   /* Get physical address of free CAN RAM write element buffer */
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x60);
   ...
   switch (get_data_count_id)
   {
   case 0 :
      send_data_id_n = 0x0;
      break;
   case 1 :
      send_data_id_n = 0x200;
      break;
   case 2 :
      send_data_id_n = 0x400;
      break;
   case 3 :
      send_data_id_n = 0x600;
      break;
   default:
      break;
   }

   get_data_frame.s_identifier = OVSP_GET_DATA_ID + send_data_id_n;
   get_data_frame.s_unused1 = (uint32_t)0x0;
   get_data_frame.s_unused2 = (uint32_t)0x0;
   get_data_frame.s_length = OVSP_FRAME_TOT_LENGTH;

   tmp_can_frame = (ts_can_frame*)(&get_data_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = get_data_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = get_data_frame.s_length;
   ...
   swap_data_cmd(&(get_data_frame.s_unused1), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   /*can_write_start_tx(can_x_reg_addr, fifo_elt_offset);*/
   can_write_start_tx(can_x_reg_addr, 6);
}
