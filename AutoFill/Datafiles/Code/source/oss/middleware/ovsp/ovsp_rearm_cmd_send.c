...
{
   ts_ovsp_rearm_cmd_frame rearm_cmd_frame;
   ...
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x50);
   ...
   rearm_cmd_frame.s_identifier = (uint32_t)OVSP_REARM_CMD_ID;

   if(rearm_request == TRUE)
   {
      rearm_cmd_frame.s_data_os_key = (uint32_t)0xF1F2F3F4;
   }
   else
   {
      rearm_cmd_frame.s_data_os_key = (uint32_t)0x0;
   }


   rearm_cmd_frame.s_data_unused = (uint32_t)0x00;

   rearm_cmd_frame.s_length = OVSP_FRAME_TOT_LENGTH;

   tmp_can_frame = (ts_can_frame*)(&rearm_cmd_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = rearm_cmd_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = rearm_cmd_frame.s_length;
   ...
   swap_data_cmd(&(rearm_cmd_frame.s_data_os_key), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 5);
}
