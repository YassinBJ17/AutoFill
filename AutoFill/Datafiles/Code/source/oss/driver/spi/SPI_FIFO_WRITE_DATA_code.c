...
void SPI_FIFO_WRITE_DATA( CMN_IOSP_ID_t       const p_spi_conf_id,
                          ...
                          CMN_SYSTEM_SIZE_t * const p_written_size)
{
   ...
   *p_written_size = 0ul;
   ...
   if ( (v_data_nb <= SPI_CFEX_FIFO_SIZE) && (p_buff_size >= (v_data_nb*SPI_CFEX_DATA_SIZE)) )
   {
      ...
      pt_data_write = (uint16_t*)p_buff_addr;
      /* */
      for ( v_i=0ul; v_i<v_data_nb; v_i++ )
      {
         ...
         if ( pt_spi_reg_descr->s_SR.u_B.s_TXCTR < (reg32_t)SPI_CFEX_FIFO_SIZE )
         {
            ...
            /* >RELAX<SYNCHRONe_C_Code_10.7> The cast is safe. There is no loss of precision from (uint16_t) to (reg32_t). */
            pt_spi_reg_descr->s_PUSHR.u_PUSHR.u_R = (reg32_t)SPI_CFEX_descriptor_tab[p_spi_conf_id].s_command_value + (reg32_t)*pt_data_write;
            /* */
            *p_written_size += SPI_CFEX_DATA_SIZE;
            /* Pointer next Data */
