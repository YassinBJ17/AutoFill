...
/* Load Type Description */
    v_LoadTypeOffset = p_LUH_ConstantSizeFirstFields->s_PointerToLoadTypeDescriptionLength * (uint32_t)2;
    if ( v_LoadTypeOffset > v_LUH_Length )
    {
        return ( E_LIBUTI_PARSE_LUH_KO_LOADTYPE_POINTER );
    }

    if (v_LoadTypeOffset != (uint32_t)0)
    {
        *p_LoadTypeDescriptionLength = *((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_LoadTypeOffset)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
        if (*p_LoadTypeDescriptionLength > (uint16_t)0)
        {
            BOOT_LIBMEM_CPY(
                ...
                (uint8_t*)((uint32_t)p_LUH_RAM_Address + v_LoadTypeOffset + (uint32_t)2), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
                (uint32_t)*p_LoadTypeDescriptionLength );
            *p_LoadTypeID = *((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_LoadTypeOffset + (uint32_t)2 + (uint32_t)(*p_LoadTypeDescriptionLength) + ((uint32_t)(*p_LoadTypeDescriptionLength) % (uint32_t)2))); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
        }
    }

/* Load check */
    v_LoadCheckOffset = p_LUH_ConstantSizeFirstFields->s_PointerToLoadCheckValueLength * (uint32_t)2;
    if ( v_LoadCheckOffset > v_LUH_Length )
    {
        return ( E_LIBUTI_PARSE_LUH_KO_LOADCHK_POINTER );
    }

    if (v_LoadCheckOffset != (uint32_t)0)
    {
        *p_LoadCheckValueLength = *((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_LoadCheckOffset)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
        if (*p_LoadCheckValueLength > (uint16_t)0)
        {
            *p_LoadCheckValueLength -= (uint16_t)4;
            v_CheckType = *((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_LoadCheckOffset + (uint32_t)2)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
            ...
            BOOT_LIBMEM_CPY(
                p_LoadCheckValue,
                (uint8_t*)((uint32_t)p_LUH_RAM_Address + v_LoadCheckOffset + (uint32_t)4), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
