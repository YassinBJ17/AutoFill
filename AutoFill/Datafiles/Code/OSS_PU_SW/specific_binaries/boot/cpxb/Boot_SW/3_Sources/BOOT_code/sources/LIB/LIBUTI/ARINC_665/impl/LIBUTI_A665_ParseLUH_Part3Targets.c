...
/* Target HW IDs with Positions */
    v_TargetHW_ID_WithPositions_ListOffset = p_LUH_ConstantSizeFirstFields->s_PointerToTargetHW_ID_withPositions_List * (uint32_t)2;
    if ( v_TargetHW_ID_WithPositions_ListOffset > v_LUH_Length )
    {
        return ( E_LIBUTI_PARSE_LUH_KO_T_HW_IDS_WITH_POS_POINTER );
    }

    if ( v_TargetHW_ID_WithPositions_ListOffset != (uint32_t)0 )
    {
        ...
        *p_NumberOfTargetHW_ID_WithPositions = (uint32_t)*((uint16_t*)((uint32_t)p_LUH_RAM_Address + v_TargetHW_ID_WithPositions_ListOffset)); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Reading value in memory */
        ...
        p_TargetHW_ID_With_Pos_Iterator = (uint8_t*)( (uint32_t)p_LUH_RAM_Address + v_TargetHW_ID_WithPositions_ListOffset + (uint32_t)2 ); /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
...
/* {{RELAX<SYNCHRONe_C_Code_17.4> Need to access the table dynamically as size is not known at compile time */
        for ( v_index = (uint32_t)0; v_index < *p_NumberOfTargetHW_ID_WithPositions; v_index++ )
        {
            ...
            if ( p_TargetHW_ID_WithPositions[v_index].TargetHW_ID_WithLength.Length > (uint16_t)K_TARGET_HW_ID_MAX_LENGTH )
            {
            ...
            BOOT_LIBMEM_CPY(
                ...
                (uint8_t*)( (uint32_t)p_TargetHW_ID_With_Pos_Iterator + (uint32_t)2 ), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
            ...
            if ( p_TargetHW_ID_WithPositions[v_index].NumberOfTargetHW_ID_Positions > (uint16_t)K_MAX_TARGET_HW_ID_POSITIONS )
            {
            ...
            for ( v_index_bis = (uint32_t)0; v_index_bis < (uint32_t)p_TargetHW_ID_WithPositions[v_index].NumberOfTargetHW_ID_Positions; v_index_bis++ )
            {
                ...
                if ( p_TargetHW_ID_WithPositions[v_index].TargetHW_ID_Positions[v_index_bis].Length > (uint16_t)K_TARGET_HW_ID_POSITIONS_MAX_LENGTH )
                {
                ...
                BOOT_LIBMEM_CPY(
                    ...
                    (uint8_t*)( (uint32_t)p_TargetHW_ID_Pos_Iterator + (uint32_t)2 ), /* %RELAX<SYNCHRONe_C_Code_10.7,SYNCHRONe_C_Code_11.1> Using as memory address */
