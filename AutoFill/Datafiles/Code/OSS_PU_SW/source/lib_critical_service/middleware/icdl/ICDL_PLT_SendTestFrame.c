...
#define K_TEST_CMD_UDP_PORTS_SRC  49300
#define K_TEST_CMD_UDP_PORTS_DST  49300


/* ---------- provided operation bodies: ------------------------------------ */
void ICDL_PLT_SendTestFrame( CMN_SYSTEM_ADDR_t const p_buff_addr,
...

{
    ...
    */
   t_command   *eventBlock_cmd = ICDL_LG_TriggerCommand(ID_CMD_TEST, (uint16_t)p_buff_size);

   /*
    * Set channel transmission
   */
   eventBlock_cmd->st_descriptor->ethUdpInfo.s_SrcPort = K_TEST_CMD_UDP_PORTS_SRC ;
   eventBlock_cmd->st_descriptor->ethUdpInfo.s_DstPort = K_TEST_CMD_UDP_PORTS_DST ;

   {
      ...
      {
         ICDL_LG_ComputeMsgArgSet((int8_t *)1, (uint32_t)4, eventBlock_cmd);
