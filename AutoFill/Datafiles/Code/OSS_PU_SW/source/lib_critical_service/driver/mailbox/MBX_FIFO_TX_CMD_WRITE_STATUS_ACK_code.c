 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Tx cmd write status ack function
 * -------------------------------------------------------------------------- */
...
/* */
void MBX_FIFO_TX_CMD_WRITE_STATUS_ACK ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id,
                                        tu_TX_CMD_FIFO_STATUS_ACK_REG * const p_status_value )
{
   /* Local Variable Declaration */
   ts_MBX_TX_CMD_REG volatile * pt_fifo_tx_cmd_write_status_ack;
   ...
   pt_fifo_tx_cmd_write_status_ack = (ts_MBX_TX_CMD_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_cmd_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_fifo_tx_cmd_write_status_ack->s_TX_CMD_FIFO_STATUS_ACK_REG = (reg32_t)p_status_value->u_R;
}
