 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Rx write config1 function
 * -------------------------------------------------------------------------- */
...
/* */
void MBX_FIFO_RX_WRITE_CONFIG1 ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id,
                                 tu_RX_FIFO_CONFIG1_REG * const p_status_value )
{
   /* Local Variable Declaration */
   ts_MBX_RX_REG volatile * pt_rx_fifo_config1;
   ...
   pt_rx_fifo_config1 = (ts_MBX_RX_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_rx_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_rx_fifo_config1->s_RX_FIFO_CONFIG1_REG = (reg32_t)p_status_value->u_R;
}
