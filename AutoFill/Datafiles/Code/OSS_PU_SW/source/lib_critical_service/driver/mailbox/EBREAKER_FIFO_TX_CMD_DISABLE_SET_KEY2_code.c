 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Tx cmd disable key2 function
 * -------------------------------------------------------------------------- */
...
/* ---------- internal constants: ------------------------------------------- */
#define F_FIFO_TX_DISABLE_SET_KEY2 (reg32_t)0x5A2C9B6C

/* ---------- internal data: ------------------------------------------------ */

/* ---------- provided operation bodies: ------------------------------------ */
/* */
void EBREAKER_FIFO_TX_CMD_DISABLE_SET_KEY2 ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id )
{
   /* Local Variable Declaration */
   ts_MBX_TX_CMD_REG volatile * pt_tx_disable_set_key_2_reg;
   ...
   pt_tx_disable_set_key_2_reg = (ts_MBX_TX_CMD_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_cmd_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_tx_disable_set_key_2_reg->s_TX_ENABLE_CLEAR_KEY_2_REG = F_FIFO_TX_DISABLE_SET_KEY2;
