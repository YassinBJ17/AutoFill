 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Rx read status function
 * -------------------------------------------------------------------------- */
...
/* */
void MBX_READ_STATUS ( CMN_IOSP_ID_t const p_conf_ip_id,
                       tu_MBX_RX_FIFO_STATUS_REG * const p_status_value )
{
   /* Local Variable Declaration */
   ts_MBX_RX_REG volatile * pt_mbx_rx_fifo_status_reg;
   ...
   pt_mbx_rx_fifo_status_reg = (ts_MBX_RX_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_conf_ip_id].s_mbx_rx_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   p_status_value->u_R = (uint32_t)(pt_mbx_rx_fifo_status_reg->s_RX_FIFO_STATUS_REG);
}
