 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : MAC ETH Set MAC SRC function
 * -------------------------------------------------------------------------- */
...
void MAC_ETH_SET_MAC_SRC  ( CMN_IOSP_ID_t const p_conf_ip_id,
                            const MAC_ETH_MACAddr_t * p_mac_addr )
{
   ...
   {
      pt_mac_eth_rx_filtre_reg->s_ETH_RX_FILTRE_SRC_REG[v_i].s_ETH_RX_FILTRE_ADR_H_REG = ( ((reg32_t)(*p_mac_addr)[ 0 ]) << 8u ) |
                                                                                         ( ((reg32_t)(*p_mac_addr)[ 1 ]) << 0u );

      pt_mac_eth_rx_filtre_reg->s_ETH_RX_FILTRE_SRC_REG[v_i].s_ETH_RX_FILTRE_ADR_L_REG = ( ((reg32_t)(*p_mac_addr)[ 2 ]) << 24u ) |
