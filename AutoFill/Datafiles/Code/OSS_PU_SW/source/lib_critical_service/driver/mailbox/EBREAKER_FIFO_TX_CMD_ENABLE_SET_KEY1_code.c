 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : Mailbox Tx cmd enable key1 function
 * -------------------------------------------------------------------------- */
...
/* ---------- internal constants: ------------------------------------------- */
#define F_FIFO_TX_ENABLE_SET_KEY1 (reg32_t)0x5596B868

/* ---------- internal data: ------------------------------------------------ */

/* ---------- provided operation bodies: ------------------------------------ */
/* */
void EBREAKER_FIFO_TX_CMD_ENABLE_SET_KEY1 ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id )
{
   /* Local Variable Declaration */
   ts_MBX_TX_CMD_REG volatile * pt_tx_enable_set_key_1_reg;
   ...
   pt_tx_enable_set_key_1_reg = (ts_MBX_TX_CMD_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_cmd_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_tx_enable_set_key_1_reg->s_TX_ENABLE_SET_KEY_1_REG = F_FIFO_TX_ENABLE_SET_KEY1;
