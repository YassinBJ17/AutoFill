...
/* */
void MBX_FIFO_TX_WRITE_CONTROL ( CMN_IOSP_ID_t const p_MBX_FIFO_conf_id,
                                 tu_TX_FIFO_CONTROL_REG * const p_status_value )
{
   /* Local Variable Declaration */
   ts_MBX_TX_REG volatile * pt_tx_fifo_control_reg;
   ...
   pt_tx_fifo_control_reg = (ts_MBX_TX_REG volatile *)c_MBX_CFEX_CONFIGURATION_tab[p_MBX_FIFO_conf_id].s_mbx_tx_fifo_address;

   /* Return Status value */
   /* >RELAX<SYNCHRONe_C_Code_10.7> The cast changes register type to data type with same size */
   pt_tx_fifo_control_reg->s_TX_FIFO_CONTROL_REG = (reg32_t)p_status_value->u_R;
}
