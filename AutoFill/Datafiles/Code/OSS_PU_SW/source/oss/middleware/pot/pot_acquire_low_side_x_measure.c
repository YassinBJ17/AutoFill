 ...
 */
float32_t pot_acquire_low_side_x_measure(
                                const te_CFEX_ADC_INT_ANx_CONF_ID x_pot_to_read)
{
   uint16_t  i;
   float32_t result;
   int16_t v_samples[POT_N_SAMPLES];
   /* Read 12 samples of the value */
   for (i = 0 ; i < POT_N_SAMPLES ; i++)
   {
      ADC_BUFFER_READ( x_pot_to_read, (CMN_SYSTEM_ADDR_t)(&(v_samples[i])) );
   }
   /* Compute and return the mean of the central 8 values */
   result = pot_compute_mean_int(v_samples);
