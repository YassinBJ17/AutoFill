...
/* ---------- internal data: ------------------------------------------------ */
float32_t pot_ls_voltage_3_raw[POT_N_SAMPLES];
...
float32_t pot_acquire_low_side_3_measure(void)
{
   uint16_t  i;
   float32_t result;
   int16_t pot_ls_int_3[POT_N_SAMPLES];

   /* Read 12 samples of the value */
   for (i = 0 ; i < POT_N_SAMPLES ; i++)
   {
      ADC_BUFFER_READ(e_CFEX_ADC_INT_AN79, (CMN_SYSTEM_ADDR_t)(&(pot_ls_int_3[i])) );
      pot_ls_voltage_3_raw[i] = ((float32_t) pot_ls_int_3[i]) * POT_LOW_SIDE_INT_TO_FLOAT;
   }
   /* Compute and return the mean of the central 8 values */
   result = pot_compute_mean_int(pot_ls_int_3);
