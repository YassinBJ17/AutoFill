...
{
   ts_ovsp_cam_cmd_frame cam_cmd_frame;
   ...
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF+ 0x30);
   ...
   cam_cmd_frame.s_identifier = (uint32_t)OVSP_CAM_CMD_ID;

   switch(cam_cmd)
   {
      case e_ovsp_cam_cmd1_off :
         cam_cmd_frame.s_data_os_key = (uint32_t)0x0A0A0A0A;
         cam_cmd_frame.s_data_as_key = (uint32_t)shmem.s_ovs.s_ovs_cmd1_off;
         break;
      case e_ovsp_cam_cmd1_on :
         cam_cmd_frame.s_data_os_key = (uint32_t)0xA1A1A1A1;
         cam_cmd_frame.s_data_as_key = (uint32_t)shmem.s_ovs.s_ovs_cmd1_on;
         break;
      case e_ovsp_cam_cmd2_off :
         cam_cmd_frame.s_data_os_key = (uint32_t)0x0B0B0B0B;
         cam_cmd_frame.s_data_as_key = (uint32_t)shmem.s_ovs.s_ovs_cmd2_off;
         break;
      case e_ovsp_cam_cmd2_on :
         cam_cmd_frame.s_data_os_key = (uint32_t)0xB1B1B1B1;
         cam_cmd_frame.s_data_as_key = (uint32_t)shmem.s_ovs.s_ovs_cmd2_on;
         break;
      default :
         /* any other values = Invalid */
         break;
   }
   cam_cmd_frame.s_length = OVSP_FRAME_TOT_LENGTH;
   tmp_can_frame = (ts_can_frame*)(&cam_cmd_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = cam_cmd_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = cam_cmd_frame.s_length;
   ...
   swap_data_cmd(&(cam_cmd_frame.s_data_os_key), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 3);
}
