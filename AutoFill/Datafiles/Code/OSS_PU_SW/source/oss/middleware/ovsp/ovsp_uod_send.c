...
void ovsp_uod_send(uint32_t const can_x_reg_addr, uint32_t const dest_can_ram_addr)
{
   ...
   /*ts_can_frame * uod_tx_buff;*/

   /* Get physical address of free CAN RAM write element buffer */
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x80);
   ...
   /*tmp_can_frame = (ts_can_frame*)(&uod_tx_buff);*/

   /* If can 3 (CAN LS) */
   if (can_x_reg_addr == 0xFFEEC000)
   {
      tmp_can_frame = &(uod_bypass.uod_can_ovsp_tx_buffer_ls);
   ...
   {
      tmp_can_frame = &(uod_bypass.uod_can_ovsp_tx_buffer_hs);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = tmp_can_frame->s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = tmp_can_frame->s_length;
   ...
   swap_data(tmp_can_frame->s_data, p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 8);
}
