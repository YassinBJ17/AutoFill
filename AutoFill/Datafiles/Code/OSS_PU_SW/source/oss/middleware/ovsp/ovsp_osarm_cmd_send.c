...
{
   ts_ovsp_osarm_cmd_frame osarm_cmd_frame;
   ...
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x40);
   ...
   osarm_cmd_frame.s_identifier = (uint32_t)OVSP_OSARM_CMD_ID;

   if(osarm_request == TRUE)
   {
      osarm_cmd_frame.s_data_os_key = (uint32_t)0xE1E2E3E4;
   }
   else
   {
      osarm_cmd_frame.s_data_os_key = (uint32_t)0x0;
   }

   osarm_cmd_frame.s_data_unused = (uint32_t)0x00;

   osarm_cmd_frame.s_length = OVSP_FRAME_TOT_LENGTH;

   tmp_can_frame = (ts_can_frame*)(&osarm_cmd_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = osarm_cmd_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = osarm_cmd_frame.s_length;
   ...
   swap_data_cmd(&(osarm_cmd_frame.s_data_os_key), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 4);
}
