...
{
   ts_ovsp_one_sensor_cmd_frame one_sensor_cmd_frame;
   ...
   local_tmp = (dest_can_ram_addr + CAN_TX_CAN_OFF + 0x70);
   ...
   one_sensor_cmd_frame.s_identifier = (uint32_t)OVSP_ONE_SENSOR_CMD_ID;

   if(one_sensor_cmd == e_one_ng_sensor_auth)
   {
      one_sensor_cmd_frame.s_data_ntl_sensor_auth = (uint32_t)0x0;
      one_sensor_cmd_frame.s_data_ng_sensor_auth  = (uint32_t)0xFFFF1111;
   }
   else if (one_sensor_cmd == e_one_ntl_sensor_auth)
   {
      one_sensor_cmd_frame.s_data_ntl_sensor_auth = (uint32_t)0xEEEE1111;
      one_sensor_cmd_frame.s_data_ng_sensor_auth  = (uint32_t)0x0;
   }

   /* TBD No two sensore config command in shmem ? is it used ?
   else if(one_sensor_cmd == e_ovsp_two_sensor_config)
   {
      one_sensor_cmd_frame.s_data_ntl_sensor_auth = (uint32_t)0xEEEE2222;
      one_sensor_cmd_frame.s_data_ng_sensor_auth  = (uint32_t)0xFFFF2222;
   }*/

   one_sensor_cmd_frame.s_length = OVSP_FRAME_TOT_LENGTH;

   tmp_can_frame = (ts_can_frame*)(&one_sensor_cmd_frame);
   ...
   if (is_fifo_full == OVSP_FIFO_NOT_FULL)
   {
      ...
      /* load std identifier */
      can_buff_tx_elt.s_tx_buff_elt_f0.u_bit.s_id = one_sensor_cmd_frame.s_identifier << CAN_STD_ID_SHIFT;
   }
   ...
   can_buff_tx_elt.s_tx_buff_elt_f1.u_bit.s_dlc = one_sensor_cmd_frame.s_length;
   ...
   swap_data_cmd(&(one_sensor_cmd_frame.s_data_ntl_sensor_auth), p_can_ram_elt->s_data);

   /* Launch CAN transmission */
   can_write_start_tx(can_x_reg_addr, 7);
}
