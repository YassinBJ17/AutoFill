 ...
 * --------------------------------------------------------------------------
 * DESCRIPTION : logbook_periodic_logs_can function which allows logbook management at
 *               each RTC
 * -------------------------------------------------------------------------- */
...
void logbook_periodic_logs_can(void)
{
   ...
   /* CAN TX log */
   logbook_fill_event_block(&log, FCT_ID_CAN_TX, shmem.s_can1_tx.s_fault_word.u_word,
                           exception_context, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE,
                           int_ext_temp, SPEED_PREBLUE_VALUE);
   logbook_save_log(&log);

   /* CAN RX log */
   logbook_fill_event_block(&log, FCT_ID_CAN_RX, shmem.s_can1_rx.s_fault_word.u_word,
                           exception_context, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE,
                           int_ext_temp, SPEED_PREBLUE_VALUE);
   logbook_save_log(&log);

   /* ECDRDL TX log */
   logbook_fill_event_block(&log, FCT_ID_ECDRDL_TX, shmem.s_can2_tx.s_fault_word.u_word,
                           exception_context, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE,
                           int_ext_temp, SPEED_PREBLUE_VALUE);
   logbook_save_log(&log);
   ...
   /* ECDRDL RX log */
   logbook_fill_event_block(&log, FCT_ID_ECDRDL_RX, shmem.s_can2_rx.s_fault_word.u_word,
                           exception_context, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE, SPARE_VALUE,
                           int_ext_temp, SPEED_PREBLUE_VALUE);
   logbook_save_log(&log);
}
